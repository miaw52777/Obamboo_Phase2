...
<?php 
	...
	$product_cat_id = getParentIDByName($proot); // product category id	
	...
	{
		$pname = trim($returnMsg['ITEM'][$i]['產品名稱']);
		$pseq = trim($returnMsg['ITEM'][$i]['排序']);
		$filepath_1 = trim($returnMsg['ITEM'][$i]['圖片檔名1']);
		$filepath_2 = trim($returnMsg['ITEM'][$i]['圖片檔名2']);
		$filepath_3 = trim($returnMsg['ITEM'][$i]['圖片檔名3']);
		$filepath_4 = trim($returnMsg['ITEM'][$i]['圖片檔名4']);
		...
		{
			$msg = 'Error : Product name must be given.';
		}
		else
		{
			$msg = insertProductAndFilepath($product_cat_id,$pname,$pvalid,$pseq,'',$filepath_1,$filepath_2,$filepath_3,$filepath_4);
